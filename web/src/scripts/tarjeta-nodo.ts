...
function iniciar(tarjeta: HTMLElement, cuerpo: HTMLElement) {
  ...
  async function mostrar(nodo: SVGGraphicsElement, comoHoja: boolean) {
    ...
    // Quién recibe el puntero lo decide el CSS por `data-modo` —ver
    // `global.css`, «Tarjeta emergente del nodo»—. Acá se intentó con una
    // clase de utilidad y no servía: `#tarjeta-nodo[data-modo=…]` tiene más
    // especificidad y ganaba siempre. Un mecanismo, no dos.
